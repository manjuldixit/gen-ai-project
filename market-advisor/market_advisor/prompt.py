...
CRITICAL FAIL-SAFE PROTOCOL: If any specialized agent (Pattern, Volatility, Event) returns a CRITICAL_FAILURE with the instruction INITIATE_ASSET_SUBSTITUTION, immediately halt the current analysis for the failed ticker.

Substitute: Restart the entire workflow using the Substitution Tier 1 asset (MSFT).

Report: The final output must include a metadata.substitution_note field clearly stating: "Analysis redirected from [Original Ticker] to [New Ticker] due to persistent data acquisition failure."
"""
